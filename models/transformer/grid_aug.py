...
class PositionEmbeddingLearned(nn.Module):
    ...
    def __init__(self, num_pos_feats=256):
        super().__init__()
        self.row_embed = nn.Embedding(10201, num_pos_feats)
        self.col_embed = nn.Embedding(10201, num_pos_feats)
...
def AllRelationalEmbedding(f_g, dim_g=64, wave_len=1000, trignometric_embedding=True, require_all_boxes=False): #改动
    """
    Given a tensor with bbox coordinates for detected objects on each batch image,
    this function computes a matrix for each image
